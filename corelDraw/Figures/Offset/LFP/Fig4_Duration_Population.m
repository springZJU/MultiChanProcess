 clear; clc
protocol = "Offset_Duration_Effect_16ms_Reg_New";
...
if strcmpi(protocol, "Offset_Duration_Effect_4ms_Reg_New")
    Offset(9:10) = [];
elseif strcmpi(protocol, "Offset_Duration_Effect_16ms_Reg_New")
    Offset(end-1:end) = [];
end
mkdir(SAVEPATH);
...
MeanLFP = changeCellRowNum(cellfun(@(x) cell2mat(cellfun(@(y) mean(y, 1), changeCellRowNum(x.rawWave), "UniformOutput", false)), {tempLFP.rawLFP}', "UniformOutput", false));
...
%     if pIndex == 1
%         base = cell2mat(cellfun(@(x) mean(x(~ismember(1:length(tempLFP), pIndex), :)), MeanLFP, "UniformOutput", false));
%     else
        base = cell2mat(cellfun(@(x) mean(x(end-2:end, :), 1), MeanLFP, "UniformOutput", false));
%     end
    temp = cellfun(@(x) x-base, temp, "UniformOutput", false);

    data = cellfun(@(x) mean(x)', changeCellRowNum(temp), "UniformOutput", false);
    
    [~, rspRMS] = cellfun(@(x) waveAmp_Norm(x, Window, rspWin+Offset(pIndex)), temp, 'UniformOutput', false);
...
%     realRMS = changeCellRowNum(cellfun(@(x, y) x-y, rspRMS, baseRMS, "UniformOutput", false));
    ...
    sponRMS = changeCellRowNum(cellfun(@(x) waveAmp_Norm(x, Window, sponWin), temp, 'UniformOutput', false));
...
[~, RMS(rIndex).Sg.maxICI] = max(RMS(rIndex).Sg.mean(1:end-1, 1));
[~, RMS(rIndex).Sg.zscoredMaxICI] = max(RMS(rIndex).Sg.zscoredMean(1:end-1, 1));
[~, RMS(rIndex).Gr.maxICI] = max(RMS(rIndex).Gr.mean(1:end-1, 1));
[~, RMS(rIndex).Gr.zscoredMaxICI] = max(RMS(rIndex).Gr.zscoredMean(1:end-1, 1));
[~, RMS(rIndex).Ig.maxICI] = max(RMS(rIndex).Ig.mean(1:end-1, 1));
[~, RMS(rIndex).Ig.zscoredMaxICI] = max(RMS(rIndex).Ig.zscoredMean(1:end-1, 1));
end
...
if strcmpi(protocol, "Offset_Duration_Effect_4ms_Reg_New")
temp = cellfun(@(x) find(x(1:end-3)==1, 1, "last"), num2cell(temp, 1), "UniformOutput", false);
elseif strcmpi(protocol, "Offset_Duration_Effect_16ms_Reg_New")
    temp = cellfun(@(x) find(x(1:end-1)==1, 1, "last"), num2cell(temp, 1), "UniformOutput", false);
end
