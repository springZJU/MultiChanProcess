clear; clc
protocol = "Offset_Duration_Effect_4ms_Reg_New";
...
for pIndex = 1 : length(tempLFP)
    ...
    MeanLFPTemp = cell2mat(cellfun(@(x) mean(x(end-2:end, :), 1), MeanLFP, "UniformOutput", false));
    plotLFP(pIndex).WaveDelta.Sg = [temp.t; mean(cell2mat(cellfun(@(x) mean(x(layIdx.Sg, :)-MeanLFPTemp(layIdx.Sg, :), 1), temp.rawWave, "UniformOutput", false)), 1)]';
    plotLFP(pIndex).WaveDelta.Gr = [temp.t; mean(cell2mat(cellfun(@(x) mean(x(layIdx.Gr, :)-MeanLFPTemp(layIdx.Gr, :), 1), temp.rawWave, "UniformOutput", false)), 1)]';
    plotLFP(pIndex).WaveDelta.Ig = [temp.t; mean(cell2mat(cellfun(@(x) mean(x(layIdx.Ig, :)-MeanLFPTemp(layIdx.Ig, :), 1), temp.rawWave, "UniformOutput", false)), 1)]';
end
...
for pIndex = 1 : length(tempLFP)
    ...
    plotLFP(pIndex).info = tempLFP(pIndex).info;
    [~, rspRMS] = cellfun(@(x) waveAmp_Norm(x, Window, rspWin+S1Duration(pIndex)), temp, 'UniformOutput', false);
