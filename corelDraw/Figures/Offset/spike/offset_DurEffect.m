...
for pIndex = 1 : length(chSpikeLfp)
    ...
    [frMean_OnsetRsp, frSE_OnsetRSP, countRaw_OnsetRsp] = cellfun(@(x) calFR(x, [0, 200], chSpikeLfp(pIndex).trialsRaw), {chSpikeLfp(pIndex).chSPK.spikePlot}', "UniformOutput", false);
    [frMean_OnsetSpon, frSE_OnsetSpon, countRaw_OnsetSpon] = cellfun(@(x) calFR(x, [-200, 0], chSpikeLfp(pIndex).trialsRaw), {chSpikeLfp(pIndex).chSPK.spikePlot}', "UniformOutput", false);
    ...
    [frMean_1, frSE_1, countRaw_1] = cellfun(@(x) calFR(x, spikeWin + S1Duration(pIndex), chSpikeLfp(pIndex).trialsRaw), {chSpikeLfp(pIndex).chSPK.spikePlot}', "UniformOutput", false);
