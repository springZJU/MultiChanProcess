...
for rIndex = 1 : length(popRes)
    ...
    for pIndex = 1 : length(tempCSD)
        ...
        % normalize
        baseWave = cellfun(@(x) findWithinWindow(x, temp.tWave, baseWin), changeCellRowNum(temp.Wave), "UniformOutput", false);
        ...
        fs = size(meanWave, 2)*1000/diff(Window);
        [f, Pow, P] = cellfun(@(x) mFFT_Base(x, fs, [0, 250]), zWave, "UniformOutput", false);
        f = f{1};
        Pow = cell2mat(cellfun(@(x) mean(x, 1), Pow, "UniformOutput", false));
        P = cell2mat(cellfun(@(x) mean(x, 1), P, "UniformOutput", false));
