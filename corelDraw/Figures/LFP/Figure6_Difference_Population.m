...
load("E:\MonkeyLinearArray\ProcessedData\TB_Ratio_4_4.04\popRes.mat")
excludeDate = "";

%% example CSD and MUA
popRes(matches(string(vertcat(popRes.Date)), excludeDate, "IgnoreCase", true)) = [];
baseWin = [-200, 0];
plotWin = [-50, 300];
rspWin = [0, 200];
...
PooledRsp.SgCSD = vertcat(plotCSD.SgRspPool);
PooledRsp.GrCSD = vertcat(plotCSD.GrRspPool);
PooledRsp.IgCSD = vertcat(plotCSD.IgRspPool);

PooledRsp.SgMUA = vertcat(plotMUA.SgRspPool);
PooledRsp.GrMUA = vertcat(plotMUA.GrRspPool);
PooledRsp.IgMUA = vertcat(plotMUA.IgRspPool);
